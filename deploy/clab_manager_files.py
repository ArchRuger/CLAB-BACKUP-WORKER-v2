...
def groups_of(inspection):
    groups = {}
    rows = inspection if isinstance(inspection, list) else [row for bucket in inspection.values() for row in bucket]
    if len(rows) > 10000: raise ValueError('Too many containers')
    for row in rows:
        name = row.get('lab_name', '')
        if not re.fullmatch(r'[A-Za-z0-9_][A-Za-z0-9_.-]{0,119}', name):
            raise ValueError('Invalid lab name')
        groups.setdefault(name, []).append(row)
    return groups


def collect(inspection, labels_for, reader=read_regular):
    sources = {}; remaining = TOTAL_LIMIT; deadline = time.monotonic() + 18
    for index, (name, rows) in enumerate(groups_of(inspection).items()):
        source = sources[name] = {'files': {}, 'warnings': []}
        if index >= 100 or time.monotonic() > deadline:
            ...
            continue
        try:
            paths = {row.get('absLabPath') for row in rows if row.get('absLabPath')}
            if len(paths) != 1: raise ValueError('Ambiguous topology path')
            definition = Path(paths.pop())
            if not definition.is_absolute() or definition.suffix.lower() not in ('.yaml', '.yml'):
                raise ValueError('Invalid topology path')
            container_id = rows[0].get('container_id', '')
            if not re.fullmatch(r'[a-fA-F0-9]{12,64}', container_id):
                raise ValueError('Invalid container ID')
            labels = labels_for(container_id)
            if labels.get('containerlab') != name or labels.get('clab-topo-file') != str(definition):
                raise ValueError('Conflicting deployment labels')
            node_dir = Path(labels.get('clab-node-lab-dir', ''))
            if not node_dir.is_absolute() or node_dir.name != labels.get('clab-node-name'):
                raise ValueError('Invalid generated directory')
            lab_dir = node_dir.parent
            candidates = {'definition': definition, 'annotations': Path(str(definition) + '.annotations.json'),
                          'inventory': lab_dir / 'ansible-inventory.yml', 'topology': lab_dir / 'topology-data.json'}
        except (ValueError, TypeError, KeyError, OSError, subprocess.SubprocessError):
            source['warnings'].append('Cannot verify deployment file locations. Check containerlab labels or import manually.')
            continue
        for kind, path in candidates.items():
            try:
                raw = reader(path)
                if len(raw) > remaining: raise ValueError('Total file limit exceeded')
                remaining -= len(raw)
                source['files'][kind] = {'path': str(path), 'sha256': hashlib.sha256(raw).hexdigest(),
                                         'content': base64.b64encode(raw).decode('ascii')}
            except (OSError, ValueError):
                source['warnings'].append(kind + ': missing, unreadable, changed while reading, symlink, or size limit reached.')
    return {'protocol': PROTOCOL, 'inspect': inspection, 'sources': sources}

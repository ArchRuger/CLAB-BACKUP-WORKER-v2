...
| ~~Unused names in `host_git.py` and `restore.py`~~ | resolved in follow-up 3 | | Removed after a risk review. `restore_junos.py` keeps `pending_rollback_shell()` and `COMMIT_ERROR` with a note: wiring the probe for an interrupted restore is a feature decision | | Decide whether an interrupted restore should probe the node | Opus |

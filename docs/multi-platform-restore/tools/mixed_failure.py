...
All four nodes are drifted to B and selected. While the manager works through the first nodes, somebody
else's timed change is armed on the node that is applied last (xrv9k: a small `commit confirmed` from an
ordinary CLI session). The expected, honest outcome: the first three nodes are replaced and verified, the
last one is refused by its driver ("not changed": a pending change that is not the manager's is never
touched), the job is `partial`, management stays usable everywhere, the foreign change is left alone and
rolls back by itself, and a later restore of that node alone succeeds. Nodes are applied one after
another: nothing here is atomic across nodes, and the evidence shows exactly that.
...
def main():
    ...
    while time.time() < deadline and not job:          # the job this run started, and its first node being applied
        job = next((j for j in state()['restore_jobs'] if j['id'] not in known and j['status'] == 'applying'), None)
        time.sleep(0.5)
    note('the job is applying its first node; arming a foreign timed change on the last one', job=job and job['id'])

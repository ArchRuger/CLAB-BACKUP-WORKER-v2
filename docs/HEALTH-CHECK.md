...
| Grafana telemetry dashboards | Informational when the optional stack is not installed; otherwise Grafana's health endpoint, whether Prometheus answers at all (a restarting container is reported with the Compose status and log commands) and whether it scrapes the manager's metrics endpoint (scrape errors are classified, never echoed). Opening the dashboards from the workstation remains an acceptance check |

...
community-signed). The telemetry setup installs it once, pinned, with the pinned Grafana
image's own CLI into `TELEMETRY_CONFIG_DIR/plugins`, and mounts that folder read-only, so
later restarts need no internet:
...
The setup also creates the folder the manager writes the maps into
(`TELEMETRY_MAPS_DIR`, default `/srv/containerlab-node-manager/data/telemetry/dashboards`,
owned by the manager's user) and mounts it read-only into Grafana. Its final lines tell
you whether Prometheus, Grafana and the Flow panel are ready; `bash
deploy/check-install.sh` repeats that check (*Grafana telemetry dashboards* warns when
the plugin is missing or the folder is not writable). Access to grafana.com is needed
during this setup only; the manager and the lab need none.

Upgrading from 1.23.x: run the two commands above once. Existing labs get their map on
the manager's first pass after the recreate.
...
telemetry on (the default since 1.23.0), the sequence is:
...
3. The Telemetry tab's button becomes **Open lab map in Grafana ↗**. Node and link
...
|---|---|
| A drawing (imported topology; positions come from the annotations file or the manager's default grid) | No map for that lab; *Open Grafana ↗* opens the lab overview instead. Import the topology once. |
...
| Symptom | Check |
|---|---|
| No *Lab maps* folder in Grafana | `bash deploy/check-install.sh` → *Grafana telemetry dashboards*. The setup must have run after upgrading to 1.24.0 (it adds the two mounts). |
...
| Panel shows a plugin error or an empty frame | The Flow panel is not loaded: rerun `sudo bash deploy/setup-telemetry.sh` with access to grafana.com and look at `docker compose … logs grafana`. |
| Links stay grey while the Telemetry tab shows rates | The interface name in the drawing does not resolve to the NOS name (see the table above), or the node is unmatched. The manager's map shows the same link as *no telemetry*. |

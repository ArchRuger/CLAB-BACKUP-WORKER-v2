...
Reviewed from merged main `2d34415` (1.19.0), with fixes prepared on
`codex/deployment-operation-audit`. These are reproduced source defects, not
...
| P2 | Discovery or Git SSH exit status arrives before the last stdout packets. | A valid response is truncated or rejected, affecting discovery and Git saves. | Wait for stream EOF and drain bounded stderr. Real localhost Paramiko tests reproduce delayed output, a fragmented 160 KiB response, and missing/nonzero status. Git stderr now counts toward its response limit. |
...
Confirm **Release 1.19.1**, matching helper versions and the last audit write in

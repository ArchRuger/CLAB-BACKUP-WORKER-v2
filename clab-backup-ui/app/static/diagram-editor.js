'use strict';
// Student names for the annotation types in the item picker (the stored type keys are unchanged).
const DIAGRAM_TYPE_NAMES={text:'Text',rectangle:'Box',circle:'Circle',line:'Line',group:'Group'};
...
async function editDiagram(id){
 let drawing=await(await api('/labs/'+id+'/topology')).json();if(!drawing)throw new Error('Import a map first.');
 const dialog=opDialog('op-layout-editor','Edit lab map',`<p>Add text, boxes, circles and lines, or drag devices to rearrange them. Save keeps this layout with the lab. You can also download the map file to reuse it with your lab, or export it to draw.io.</p>
 <div class="diagram-toolbar"><button class="button secondary" data-add-shape="text">Add text</button><button class="button secondary" data-add-shape="rectangle">Add box</button><button class="button secondary" data-add-shape="circle">Add circle</button><button class="button secondary" data-add-shape="line">Add line</button><button class="button secondary" id="diagram-undo" disabled>Undo</button><button class="button secondary" id="diagram-fit">Fit</button></div>
 <div class="diagram-workspace"><svg id="op-layout-map" class="topology-map op-layout-map" aria-label="Editable lab topology"></svg>
 <form id="diagram-properties" class="diagram-properties"><label>Select item<select id="diagram-selection"></select></label><p id="diagram-empty">Click a device or shape on the map, or add text or a shape.</p>
 ...
 <div class="diagram-fields"><label>Width<input data-prop="width" type="number" min="1" max="100000" step="any"></label><label>Height<input data-prop="height" type="number" min="1" max="100000" step="any"></label><label>Line end X<input data-prop="x2" type="number" min="-100000" max="100000" step="any"></label><label>Line end Y<input data-prop="y2" type="number" min="-100000" max="100000" step="any"></label><label>Font size<input data-prop="fontSize" type="number" min="6" max="160" step="any"></label><label>Text color<input data-prop="fontColor" type="color"></label><label>Fill color<input data-prop="fillColor" type="color"></label><label>Fill opacity (0 = clear, 1 = solid)<input data-prop="fillOpacity" type="number" min="0" max="1" step=".05"></label><label>Border color<input data-prop="borderColor" type="color"></label><label>Border width<input data-prop="borderWidth" type="number" min="0" max="20" step="any"></label></div>
 ...
 <button type="button" class="button danger-outline" id="diagram-delete">Remove this item</button></div></fieldset></form></div>
 <p id="diagram-state" role="status">No unsaved changes.</p><div id="diagram-discard" hidden><p>Discard your unsaved map changes?</p><button class="button secondary" id="diagram-keep">Keep editing</button><button class="button danger-outline" id="diagram-discard-confirm">Discard changes</button></div>
 <div class="dialog-actions"><button class="button secondary" id="diagram-cancel">Cancel</button><button class="button secondary" id="op-layout-json">Download map file (.annotations.json)</button><button class="button secondary" id="op-layout-export">Export to draw.io</button><button class="button primary" id="op-layout-save">Save map</button></div>`);
 ...
 const properties=()=>{
  ...
  $('diagram-selection').innerHTML='<option value="">Choose an item</option>'+drawing.nodes.map((n,i)=>`<option value="n:${i}">Device: ${esc(n.label||n.alias)}</option>`).join('')+drawing.decorations.map((d,i)=>`<option value="a:${i}">${esc(DIAGRAM_TYPE_NAMES[d.type]||d.type)}: ${esc(d.text?.slice(0,50)||'Item '+(i+1))}</option>`).join('');
 ...
 const render=()=>{
  svg.innerHTML=topologyMarkup(drawing);svg.querySelectorAll('[data-map-id]').forEach((el,i)=>{el.removeAttribute('aria-haspopup');el.dataset.diagramNode=String(i);el.classList.toggle('diagram-selected',selected==='n:'+i);el.querySelector('title').textContent='Drag to move';});
 ...
 dialog.querySelectorAll('[data-add-shape]').forEach(button=>button.onclick=()=>{
  if(drawing.decorations.length>=2000){notify('The map can hold at most 2000 text and shape items.');return;}
 ...
 const payload=()=>{if(!$('diagram-properties').reportValidity())throw new Error('Fix the highlighted field, then try again.');return diagramPayload(drawing);};
 ...
 $('op-layout-save').onclick=()=>opTask(dialog,async()=>{await json('/labs/'+id+'/layout','PUT',payload());dirty=false;dialog.close();if(typeof refreshMap==='function'&&id===activeId)await refreshMap(true);notify('Map saved.');});
 render();properties();
}

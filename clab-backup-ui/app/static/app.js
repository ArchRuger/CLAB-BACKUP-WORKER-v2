...
// Why an SSH action is disabled right now; the readiness monitor clears it on its own.
function sshHint(n){const s=n.nos_login?.status;return s==='booting'?'NOS is still booting; SSH opens when it accepts a login':s==='failed'?'SSH login failed with the saved credentials; assign a credential profile':s==='unavailable'?'Node is not running':'Assign credentials first';}
function render(){
 const lab=current();
 ...
 const version=state.version||'1.23.0';$('app-version').textContent='v'+version;
 ...
 $('subtitle').textContent=lab?'Your nodes, connections, and configuration history.':'Deploy a topology from your VM, or pick up a lab that is already running.';
 $('import-top').textContent=lab?'↑ Replace inventory':'Deploy a new lab';
 $('updated').textContent=lab?'Inventory updated '+new Date(lab.updated).toLocaleString():'No lab selected';
 ...
 $('map-ssh-all').title=lab.nodes.some(n=>n.ssh_ready)?'':lab.nodes.some(n=>n.nos_login?.status==='booting')?'Waiting for the NOS to accept SSH logins; this opens automatically':'No node is ready for SSH';
...
function nodeActions(n,details=false){return `<button data-capture="${esc(n.name)}" ${typeof captureActionAttrs==='function'?captureActionAttrs():''}>Capture</button><button class="ssh-action" data-terminal="${esc(n.name)}" ${!n.ssh_ready?`disabled title="${esc(sshHint(n))}"`:''}>SSH <span aria-hidden="true">↗</span></button><button data-backup="${esc(n.name)}" ${busy()||n.readiness!=='Ready'?'disabled title="Requires a supported NOS, credentials, and an idle worker"':''}>Back up</button>${details?`<button data-check="${esc(n.name)}" ${!(n.login_configured??n.ssh_ready)?'disabled':''}>Test login</button><button data-telemetry="${esc(n.name)}">Telemetry</button><button data-edit="${esc(n.name)}">Edit connection</button>`:`<button class="details-action" data-details="${esc(n.name)}" aria-label="Details for ${esc(n.name)}">Details <span aria-hidden="true">→</span></button>`}`;}
...
function renderJobs(){
 ...
 <summary><div class="job-title"><strong>${j.operation==='test'?'NOS login test'+(j.source==='automatic'?' · automatic':''):'Configuration backup'}</strong><small>${esc(utcDisplay(j.started||j.created))} · ${j.nodes.length} nodes</small></div>${badge(j.status)}</summary>
...
async function handleNodeAction(e){const b=e.target.closest('button');if(!b||b.disabled)return;
 ...
 if(b.dataset.telemetry&&typeof openTelemetry==='function')openTelemetry(b.dataset.telemetry);

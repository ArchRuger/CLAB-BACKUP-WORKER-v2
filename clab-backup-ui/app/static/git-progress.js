...
function gitRenderRepository(id,context,catalog){
 ...
 <p class="form-help">Git runs as the registered VM account using its existing Git login and commit identity. Configure that login on the VM outside this application.</p><p class="form-error" role="alert"></p><div class="actions"><button type="submit" class="button primary" ${supported.length?'':'disabled'}>${activeBinding?'Save repository settings':'Connect repository'}</button></div></form>`:`<div class="blank-state"><h3>No repositories registered on this VM</h3><p>From the release source directory on the VM, run guided setup as your ordinary account, without sudo:</p><pre class="git-setup-command">bash deploy/setup-git.sh</pre><p>The wizard handles checkout, Git login and registration. No additional Linux user is needed. Refresh status after setup.</p></div>`}

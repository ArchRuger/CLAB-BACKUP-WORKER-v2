...
async function api(path,options={}){const response=await fetch('/api'+path,{...options,headers:{Authorization:'Bearer '+(sessionStorage.getItem('uiToken')||''),...options.headers}});if(response.status===401){if(!$('workspace-login').open)$('workspace-login').showModal();throw new Error('Enter the manager access token.');}if(!response.ok){const value=await response.json();throw new Error(value.detail||'Request failed.');}return response;}
...
document.addEventListener('DOMContentLoaded',()=>{launchWorkspace();$('workspace-open').onclick=()=>opTask(null,()=>opBrowse(params.get('path')||'',activeId));$('workspace-history').onclick=()=>opHistory();$('workspace-login-form').onsubmit=async e=>{e.preventDefault();sessionStorage.setItem('uiToken',$('workspace-token').value);$('workspace-token').value='';$('workspace-login').close();await launchWorkspace();};});

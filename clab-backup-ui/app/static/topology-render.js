...
function topologyLink(pair,nodes,index,settings){
 ...
 if(a.x===b.x&&a.y===b.y)return `<g class="topology-wire" ${captureAttrs} data-link-index="${index}"><title>${esc(a.label+':'+pair[0].interface+' — '+b.label+':'+pair[1].interface)}</title>${wirePath(`M${a.x-12} ${a.y-20}C${a.x-60} ${a.y-75},${a.x+60} ${a.y-75},${a.x+12} ${a.y-20}`)}</g>`;

...
function topologyDecoration(d){
 ...
 if(d.type==='text'){
  ...
  content=`<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${esc(d.backgroundColor||'transparent')}"/><text x="${tx}" y="${y+fs+4}" text-anchor="${anchor}" font-size="${fs}" fill="${esc(d.fontColor||d.color||'#333')}" font-weight="${d.fontWeight||'normal'}" font-style="${d.fontStyle||'normal'}" text-decoration="${d.textDecoration||'none'}">${d.text.split('\n').map((line,i)=>`<tspan x="${tx}" dy="${i?fs*1.5:0}">${esc(line)}</tspan>`).join('')}</text>`;
 }else{
...
function topologyNode(n){
 ...
 const path=icon.includes('switch')?'M-13-8H13M-13 8H13M8-13L13-8L8-3M-8 3L-13 8L-8 13':icon.includes('server')||icon.includes('linux')||icon.includes('host')?'M-12-13H12V13H-12ZM-12-4H12M-12 4H12M-7-9H-3M-7 0H-3M-7 9H-3':'M-14 0H14M0-14V14M8-6L14 0L8 6M-6-8L0-14L6-8M-8-6L-14 0L-8 6M-6 8L0 14L6 8';
 ...
 return `<g class="map-device ${n.inventory_name?'':'unmatched'}" transform="translate(${n.x} ${n.y})" data-map-id="${esc(n.id)}" ${n.inventory_name?`data-map-node="${esc(n.inventory_name)}" tabindex="0" role="button" aria-haspopup="menu" aria-label="Open ${esc(n.label)} actions"`:''}><title>${esc(n.inventory_name?'Right-click for SSH and node actions: '+n.inventory_name:'Unmatched node — no inventory connection')}</title><g transform="rotate(${rotation})"><rect class="device-body" x="${-r}" y="${-r}" width="${size}" height="${size}" rx="${n.iconCornerRadius??4}" fill="${esc(color)}"/><path d="${path}" class="device-symbol"/></g>${pos==='none'?'':`<g class="device-label"><rect class="device-label-bg" fill="${esc(n.labelBackgroundColor||'#454545')}" rx="3"/><text x="${tx}" y="${ty}" text-anchor="${anchor}">${esc(n.label)}</text></g>`}${n.inventory_name?'':`<text class="unmatched-label" y="${r+30}" text-anchor="middle">Unmatched</text>`}</g>`;
}
function topologyLink(pair,nodes,index,settings){
 const a=nodes.get(pair[0].node),b=nodes.get(pair[1].node);if(!a||!b)return '';

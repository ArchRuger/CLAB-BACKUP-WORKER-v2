...
function page(options={}){
 ...
 const store=options.storage||storage(),calls=[],toasts=[];
 const context=vm.createContext({console,crypto:require('node:crypto').webcrypto,TextEncoder,URLSearchParams,FormData,Blob,setTimeout:()=>0,clearTimeout(){},confirm:()=>options.confirm??true,alert(){},
 ...
 return {c:context,el,store,calls,toasts,run:code=>vm.runInContext(code,context)};
}
...
test('a page that never heard how its save ended asks the VM on the next visit',async()=>{
 const p=page({answer:(url,body)=>url.endsWith('/operations/read')?(body.path===VM_PATH?{text:TOPOLOGY,sha256:'h1'}:{refused:'No such file.'}):{}});
 ...
 assert.equal(p.run('builderDraft.vm.path'),VM_PATH);assert.equal(p.el('builder-status').textContent,'Saved on the VM');
...
test('a refused save says what was kept, and offers the differences when the VM holds another version',async()=>{
 ...
 const p=page({answer:(url,body)=>url.endsWith('/operations/preview')?{refused:'A lab folder with this name already exists on the VM: /srv/labs/lab1. Choose another lab name.'}:url.endsWith('/operations/read')?(body.path===VM_PATH?{text:onVm,sha256:'raw-hash'}:{refused:'No such file.'}):{}});
 ...
 const q=page({answer:(url,body)=>url.endsWith('/operations/preview')?{refused:'exists'}:url.endsWith('/operations/read')?(body.path===VM_PATH?{text:TOPOLOGY,sha256:'h'}:{refused:'No such file.'}):{}});
...
test('one draft per lab: opening the file a draft was saved to continues that draft',async()=>{
 const p=page({answer:(url,body)=>url.endsWith('/operations/read')?(body.path===VM_PATH?{text:TOPOLOGY,sha256:'h'}:{refused:'No such file.'}):url.endsWith('/operations/parse-yaml')?{name:'lab1'}:{}});
...
test('a downloaded draft is checked before it becomes a draft, and takes the name its topology carries',async()=>{
 ...
 assert.equal(made.draft.id,'new:lab1');assert.equal(made.draft.name,'lab1');assert.equal(made.draft.annotations,'');assert.match(made.note,/layout/);
...
test('the builder opens in the folder being browsed, else in the project folder',()=>{
 ...
 const moves=[];c.location={pathname:'/static/lab-builder.html',assign:u=>moves.push(u),reload:()=>moves.push('reload')};c.opBuilderOpen({path:'/srv/l.clab.yml'});assert.deepEqual(moves,['/static/lab-builder.html#path=%2Fsrv%2Fl.clab.yml','reload']);
